...
No API key or `.env` needed — stills and clips are generated by hand in
Google Flow's UI and downloaded into `assets/` (see the skill's top-level
"Generation model" section for the halt-and-wait workflow).
